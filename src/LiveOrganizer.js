...
export default function LiveOrganizer({ isOrganizer }) {
  ...
  // countdown calcolato con deadline
  useEffect(() => {
    if (!deadline) return;
    ...
    return (
      <div style={{ textAlign: "center", marginTop: 50 }}>
        {isOrganizer ? (
          ...
          <h2>In attesa che il gioco inizi...</h2>
        )}
      </div>
    );
  }
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 50 }}>
      ...
      <p>
        <strong>Tempo rimasto:</strong> {timeLeft}s
      </p>
        ...
        {currentQuestion.options.map((opt, i) => {
          ...
          return (
            <button
              key={i}
              style={style}
              disabled={correctAnswer !== null || selectedAnswer !== null}
              onClick={() => {
                if (!isOrganizer && selectedAnswer === null) {
                  setSelectedAnswer(i);
      ...
      {/* Controlli extra solo per l'organizzatore */}
      {isOrganizer && (
